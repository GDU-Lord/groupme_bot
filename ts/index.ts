...
export async function onCommunityUpdate(community: Community) {
  await bot.TryCatch(() => {
    ...
    return bot.bot.editMessageText(newContent, {
...
function initGroupMe(_user?: TelegramBot.User) {
  return async (initMsg: Message) => {
    ...
    const buttons = bot.getButtonsMarkup(bot.arrange(groups, 4, [["Сховати", initMsg.message_id]]), [...groups.map(() => "set_group"), "close_menu"]);
    let buttons_hash = JSON.stringify(buttons);
    const menuMessage = await bot.replyToMessage(initMsg, `<a href="tg://user?id=${member.id}">@${member.username ?? member.name}</a>, обери, будь ласка, групи, до яких хочеш приєднатися та натисни "Сховати"!\n\nТи зможеш надалі змінити список своїх груп за допомогою команди /groupme`, buttons);
...
bot.addMessageListener(async msg => {
  ...
  for (const user of users) {
    bot.addMessageListener(initGroupMe(user));
  }
});